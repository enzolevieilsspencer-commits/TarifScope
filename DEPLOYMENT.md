...
## Lien avec le front Next.js (TarifScope)

Ce guide décrit le déploiement du **scraper** (back) sur Railway. Le front Next.js est déployé sur Vercel et lit les mêmes tables Supabase (`hotels`, `rate_snapshots`, `scraper_logs`).

Pour une vue d’ensemble **Back ↔ Front** (qui écrit/lit quoi, variables Vercel vs Railway, contrat API) : voir **[INTEGRATION.md](./INTEGRATION.md)** à la racine du projet TarifScope.
